...
class MinPriorityQueue(object):
    ...
    def delete(self):
        try:
            min_val = 0
            for i in range(len(self.queue)):
                if self.queue[i].dist < self.queue[min_val].dist:
                    min_val = i
...
if __name__ == '__main__':
    ...
    image = iio.imread('./images/human.png') #reading the image using opencv
    ...
    #points = [(random.randint(0,100),random.randint(0,100)) for i in range(10000)]
    ...
    for i in range(len(h)-1):
            ...
            perimeter = perimeter + edger.dist
    ...
    perimeter = perimeter + temp_edge.dist
    cutoff = 7/1000 * perimeter
    for i in range(len(all_Edges)):
        if(all_Edges[i].dist < cutoff and ([all_Edges[i].a,all_Edges[i].b] in h)):    
            h.remove([all_Edges[i].a,all_Edges[i].b])
    xs1 = [x[0] for x in h]
    ys1 = [x[1] for x in h]
    xs1.append(h[0][0])
    ys1.append(h[0][1])
